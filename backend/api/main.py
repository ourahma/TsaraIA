...
# Variables globales pour éviter de recharger les données à chaque requête
_vectorstore = None
_agent_executor = None

def initialize_rag_system():
    """Initialise le système RAG une seule fois au démarrage"""
    global _vectorstore, _agent_executor
    ...
    try:
        ...
        agent = create_tool_calling_agent(
            llm=llm, 
            tools=[tool],  # Votre outil RAG
            prompt=prompt  # Utilisez votre prompt personnalisé ici, pas le message utilisateur
        )
...
async def chat_endpoint(chat_message: ChatMessage):
    try:
        ...
        # Utiliser l'agent executor avec le message de l'utilisateur
...
async def health_check():
    global _agent_executor
    status = "healthy" if _agent_executor is not None else "unhealthy"
