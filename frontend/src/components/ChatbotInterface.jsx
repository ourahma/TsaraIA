...
const ChatbotInterface = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const botMessage = {
        id: Date.now() + 1,
        type: 'bot',
        content: data.summary,  // Le résumé principal
        topic: data.topic,      // Le sujet de recherche
        sources: data.sources,  // Les sources utilisées
        tools_used: data.tools_used,  // Les outils utilisés
        timestamp: new Date()
      };
  ...
  const TypingIndicator = () => (
    <div className="flex items-center space-x-1 p-3">
      ...
      <span className="text-sm text-gray-500 ml-2">Le bot réfléchit...</span>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-dark-red-900 via-dark-red-500 to-dark-red-900">
      {/* Banner Section */}
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0">
          <img
            ...
            style={{ opacity: 0.5 }}
          />
          <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-dark-red-500 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-pulse"></div>
          <div className="absolute top-3/4 right-1/4 w-64 h-64 bg-beige rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-pulse" style={{ animationDelay: '2s' }}></div>
          <div className="absolute bottom-1/4 left-1/2 w-64 h-64 bg-pink-500 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-pulse" style={{ animationDelay: '4s' }}></div>
        </div>
          ...
          <div className="animate-fade-in-up">
            <div className="flex justify-center mb-8">
              <div className="relative">
                <div className="w-24 h-24 bg-dark-red rounded-2xl flex items-center justify-center shadow-2xl transform rotate-4 animate-bounce">
      ...
      {/* Chat Section */}
      <section ref={chatSectionRef} className="min-h-screen bg-beige-main relative">
            ...
            <div className="h-96 overflow-y-auto p-6 space-y-4 bg-gray-900/50">
              {messages.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-400 text-lg">Commencez votre conversation...</p>
                  <p className="text-gray-500 text-sm mt-2">Posez-moi une question et j'utiliserai mon système RAG pour vous répondre</p>
                </div>
              )}
               ...
               {messages.map((message) => (
                message.type === 'user' ? (
                  <div key={message.id} className="flex items-start space-x-3 flex-row-reverse space-x-reverse animate-fade-in-up">
                    ...
                    <div className="max-w-xs lg:max-w-md px-4 py-3 rounded-2xl bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-br-md">
            ...
            <div className="border-t border-white/10 p-6">
              <div className="flex space-x-4">
                <input
                  ...
                  className="flex-1 bg-white/10 border border-white/20 rounded-2xl px-6 py-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                  disabled={isLoading}
                />
                <button
                  onClick={handleSendMessage}
                  disabled={isLoading || !inputValue.trim()}
                  className="bg-gradient-to-r from-blue-500 to-purple-600 text-white p-3 rounded-2xl hover:from-blue-600 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105 active:scale-95"
                >
